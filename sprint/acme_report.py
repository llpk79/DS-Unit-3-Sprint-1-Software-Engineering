...
def inventory_report(products: list) -> None:
    ...
    ave_weight = sum([prod.weight for prod in products]) / len(products)
    ave_price = sum([prod.price for prod in products]) / len(products)
    ave_flame = sum([prod.flammability for prod in products]) / len(products)
